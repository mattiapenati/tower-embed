...
fn expand_derive_embed(input: syn::DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    let DeriveEmbedAttrs { folder, crate_path } = attrs;

    let root = root_absolute_path(&folder);
    let embedded_files = get_files(&root).map(|file| {
...
fn get_files(root: &Path) -> impl Iterator<Item = File> {
